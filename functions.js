...
module.exports = {
  getMember : function(message, toFind = '') {
    ...
    if (!target && toFind) {
      target = message.guild.members.find(
          member => {return member.displayName.toLowerCase().includes(toFind) ||
  ...
  getGuildDomain : function(message) {
    ...
    var output = startingString.toLowerCase()
                     .replace(/[^a-zA-Z0-9 ]/g, "")
                     .split(/ +/g)
                     .join("-");
    return output;
  ...
  formatDate : function(
      date) { return new Intl.DateTimeFormat('en-US').format(date) },

  warningEmbed : function(client, message) {
    const embed = new RichEmbed().setColor("#ff8200").setDescription(
  ...
  updateBot : async function() {
    try {
      const response = await axios.get(
          'https://raw.githubusercontent.com/solemcaelum/echleon/master/package.json');
  ...
  syncGuild : async function(guildObject) {
    ...
    foundGuild =
        await Guild.findOne({id : guildObject.id})
  ...
  syncUser : async function(userObject) {
    let time = moment().unix();
    foundUser =
        await User.findOne({id : userObject.id})
            .catch(err => { console.log("[🛰️  ] [❕] | " + err); });
