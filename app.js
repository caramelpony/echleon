...
function presentTOS(guild) {
  ...
  const roleColor =
    guild.me.displayHexColor === "#000000"
      ? "#ffffff"
  ...
  const embed = new RichEmbed()
    ...
    .addField(
      "Terms:",
      stripIndents`Server, Channel, and Memeber information
  ...
  client.users.forEach((user) => {
    User.findOrCreate(
      ...
      async function (err, foundGuild, created) {
        if (created) {
          console.log(
            `New user added to database! ID: ${user.id} | Username: ${user.username}`
          );
          syncUser(user);
        } else {
          console.log(
            `User Exists! Updating database! ID: ${user.id} | Username: ${user.username}`
          );
          syncUser(user);
        }
      }
