    ...
    (async () => {
      ...
      if (args.length < 1) {
        return message.channel
          .send(
            warningEmbed(
              client,
              "Please pick a command! See `=help pr` for more."
            )
          )
          .then((m) => m.delete(5000));
      }
      ...
      if (
        ...
        await message.guild
          ...
          .then((role) => {
            member
              .addRole(role, "Assigned private role.")
          ...
          .then(
            ...
            (chan) => {
              ...
              chan
                ...
                .then(
                  // Move the text channel to the category.
                  (chan2) => {
        ...
        if (
          message.channel.parent == category &&
        ...
        function deleteChannel() {
          server.roles
            .find((role) => role.name === message.channel.name)
            .delete();
        ...
        message.channel.send(
          warningEmbed(
            client,
            `${member.user.username} has requested this channel be deleted! ***60*** seconds remaining until deletion. This can not be cancelled.`
          )
        );
        ...
        // Identify role.
        let foundRole = server.roles.find(
          (role) => role.name === message.channel.name
        );
        // Add role to the aforementioned user.
        if (foundMember != null) {
          foundMember
            .addRole(foundRole, "Assigned private role.")
            .catch((error) => console.log(error));
        } else {
          // Unknown Error. Don't really know why this would fail. Don't care
          // to.
          return message.channel
            .send(warningEmbed(client, "Unkown Error!"))
            .then((m) => m.delete(5000));
        }
      } else if (
        args[0] == "kick" ||
        ...
        if (foundMember != null) {
          foundMember
            .removeRole(foundRole, "Removed private role.")
            .catch((error) => console.log(error));
        } else {
          return message.channel
            .send(warningEmbed(client, "Unkown Error!"))
