const { getMember, warningEmbed, formatDate, getUserDomain, getGuildDomain } = require("../../functions.js");
...
module.exports = {
    ...
    run: async (client, message, args) => {
        (async () => {
            ...
            if (!category) {
                return message.channel.send(warningEmbed(client,"No category called `Private Channels`. Please ask a server admin to fix this.")).then(m => m.delete(5000));
            }
            
            // Make sure that the subcommands are used.
            if (args.length < 1) {
                return message.channel.send(warningEmbed(client,"Please pick a command! See `=help pr` for more.")).then(m => m.delete(5000));
            }

            // Long ElseIf chain to determine what to do.
            // We start with the creation command.
            if (args[0] == "new" || args[0] == "n" || args[0] == "create" || args[0] == "c") {
                var roominstance = `${randomString(6, '#a')}`;
                var tempRoomName = `${getUserDomain(member)}-${roominstance}`;
                var roomname = tempRoomName.toLowerCase();

                // Because we're allowing the command to be used such as "=pr @CaramelDrop",
                // we need to assign the role to both users, so they can both enter chat.
                ...
                await message.guild.createRole({
                    // Role Settings
                        name: roomname,
                        permissions: []
                }, "Created private role.").then(role => {
                ...
                // Create Channel
                server.createChannel(roomname, 'text').then( // Create the actual text channel.
                    (chan) => {
                        // Take the newly create channel "chan" and assign it to the proper category.
                        chan.setParent(category.id).then( // Move the text channel to the category.
                            (chan2) => {
                                ...
                                chan2.overwritePermissions(message.guild.roles.find('name', roomname), {
                                ...
                                chan2.send(warningEmbed(client,"Remeber, admins can see your private chat!")).then(m => m.delete(60000));
                            }
                        ).catch(console.error);
                    }
                ).catch(console.error);
                ...
                function findInvitee(targetMember) {
                    ...
                    if (message.mentions.members) {
                        ...
                        return message.channel.send(warningEmbed(client,"Please mention a user (e.g. `@Echleon`) to invite! ")).then(m => m.delete(5000));
                    }
                ...
                function findInvitee(targetMember) {
                    ...
                    if (message.mentions.members) {
                        ...
                        return message.channel.send(warningEmbed(client,"Please mention a user (e.g. `@Echleon`) to kick! ")).then(m => m.delete(5000));
                    }
                ...
                message.channel.send(warningEmbed(client,"Please pick a valid command! See `=help pr` for more.")).then(m => m.delete(5000));
            }
